...
- 상시상품(전체상품) 장바구니 할인가 미적용 버그 수정
  - 사이드 카트 / 예약 페이지 / 예약 확인 페이지 모두 `product.discountPrice` 우선 적용
  - 합계 계산식에 `product.discountPrice` fallback 추가
  - 쿠키 백업(Safari ITP 대응)에 `product.discountPrice` 포함
  - 상품 상세 페이지의 일반 상품도 정상가/할인가 동시 표시 (이벤트 상품과 동일 UX)
- 예약 페이지 하단에 총 금액(부가세 별도) 섹션 추가
